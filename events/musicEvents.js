...
player.events.on('playerError', async (error, track) => {
	logger.guildLog(track.queue.guild.id, "error", [error, "Player encountered an playerError"])
	logger.guildLog(track.queue.guild.id, "info", "Track has an error, skipping");
